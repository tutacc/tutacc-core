...
// Session is session based settings for controlling Tutacc requests. It contains various settings (or limits) that may differ for different users in the context.
type Session struct {
...
type Manager interface {
	...
	// ForSystem returns the System policy for Tutacc system.
	ForSystem() System
}
...
func init() {
	const key = "tutacc.ray.buffer.size"
